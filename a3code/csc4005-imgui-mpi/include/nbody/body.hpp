...
public:
    ...
    public:
        ...
        double distance(Body &that) {// distance between 2 balls
        ...
        void update_for_tick( // update status for one single ball (v and position, not include a)
                double elapse,
...
//            std::string str="in up: "+std::to_string( elapse)+"\n";
//            std::cout<<str;
        }
    ...
    void clear_acceleration() { // all acceleration = 0
    ...
    static void check_and_update(Body i, Body j, double radius, double gravity) { // update a, v for 2 balls only, only consider the influence between these 2 balls
